...
class imageSubscriber(Node):
    ...
    def callback(self, ros_data):
        ...
        if not cv2.imwrite(os.path.join(os.path.expanduser('~'),'ros2_ws','src','turtlebot_test','testImages','test.jpg'), image_np):
...
def testCamera(args=None):
    ...
    rclpy.shutdown()
    os.system('tiv ~/ros2_ws/src/turtlebot_test/testImages/test.jpg')
